...
from tensorflow.keras.backend import sum, abs
...
class CustomModel(Model):
    def __init__(self, vision_encoder, text_encoder, decoder, words_to_idx, words_per_caption, **kwargs):
        ...
        self.sequence_concatenator_2 = Concatenate(axis=1)
    ...
    #
    #     # Input of text_encoder is of shape (None, 5, words_per_caption)
    #     # Outputs (None, 5 * words_per_caption, text_encoder_lstm_size)
    #     expected_embeddings = self.text_encoder(expected_text)
    ...
    #     return (hard_loss + 0.2 * (embedding_loss + 1)) / 2
    ...
    def compute_loss(self, expected_text, predicted_text):
        ...
        expected_text_indices = tf.cast(expected_text, tf.float64)
        predicted_text_indices = tf.cast(tf.argmax(predicted_text, axis=-1), tf.float64)
        ...
        # Return Manhattan Distance of word encodings
        result = sum(abs(expected_embeddings - predicted_embeddings), axis=-1)
